...
Unmounting is distinct from destroying the core. `editor.unmount()` idempotently destroys the active View and detaches synchronization while preserving the core's state and Editor Scope, so the live core may be mounted again. A failing View destruction is reported after detachment as `EditorUnmountError { cause }`. `core.destroy()` synchronously invalidates the core, automatically unmounts any active View, permanently rejects future mounts, and returns a shared `Promise<void>` that completes after all Editor Scope finalizers finish. A View destruction failure rejects that Promise as `EditorDestructionError { cause }` rather than escaping synchronously.
